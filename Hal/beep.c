...
void Beep_Init(void)
{
	...
	gpioInitStruct.GPIO_Pin = GPIO_Pin_0;						//将初始化的Pin脚
...
void Beep_Set(_Bool status)
{
	...
	GPIO_WriteBit(GPIOB, GPIO_Pin_0, status == BEEP_ON ? Bit_SET : Bit_RESET);		//如果status等于BEEP_ON，则返回Bit_SET，否则返回Bit_RESET
...
************************************************************
*/
void Beep_test(int ms)
{
	
		Beep_Set(BEEP_ON);mDelay(ms);		//打开蜂鸣器，并延时500ms
		
		Beep_Set(BEEP_OFF);mDelay(ms);		//关闭蜂鸣器，并延时500ms
