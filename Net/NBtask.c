...
void write_callback( uint16_t objid, uint16_t instid,uint16_t resid, nbiot_value_t *data )
{
...
void read_callback( uint16_t  objid,uint16_t instid,uint16_t  resid, nbiot_value_t *data)
{	   
...
void get_data(void)
{
	...
	ret = nbiot_resource_add( dev,3311,0,5850,1,1,&LED);
	if( ret )
	{
	...
	ret = nbiot_resource_add( dev,  3303,  0, 5700, 1, 1, &temp );//温度
	if ( ret )
	{
		nbiot_device_destroy( dev );
	...
	ret = nbiot_resource_add( dev, 3304,0, 5700,1,1,&humi );//湿度
	if ( ret )
	{
		nbiot_device_destroy( dev );
	...
	ret = nbiot_resource_add( dev,3301,0, 5700, 1, 1, &illumi );//光照
	if( ret )
	{
		nbiot_device_destroy( dev );
